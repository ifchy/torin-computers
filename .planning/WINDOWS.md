...
| 35 | 04 | unrun-verify | src/includes/spam-guard.php |  | CLOSED 2026-09-24 as STALE — it WAS authored. Commit c6cce02 'test(spam-guard): the behaviour block, executable' wrote scripts/spam-guard-selftest.php with 27 assertions, not the 9 the cutover checklist predicted. Executed for the first time on 2026-09-24 against the new PHP 8.5.10 runtime: 27/27 hold, covering the decoy field (filled/untouched/absent/array-posted), timestamp signing (forged, same-length tampered, malformed, too-fast, stale, inclusive bounds), the per-address throttle (unseen address, hashed record, inside-window refusal, outside-window pass, per-address isolation, max=0 proving the counter is read), and source-level assertions (hash_equals, no session, no superglobal in logs, no forwarded-for trust, PHP 5.2 short-array contract, no residue left behind). With this, ALL FOUR selftests named in cutover checklist 1.2 exist and pass: upload 18/18, settings 27/27, notify 9/9, spam-guard 27/27 — 81 assertions, zero unrun. | fixed |  | 2026-09-21T06:08:41.039Z | 2026-09-22T10:25:22.948Z | 2026-09-24T09:10:00.000Z |
...
| 43 | 04 | deviation | src/js/analytics.js |  | CLOSED 2026-09-24 — decided (option b) and implemented. Owner picked deploy-time JS comment stripping over raising the budget or hand-stripping. scripts/lib/strip-js-comments.py now mirrors the existing strip-css-comments.py and deploy-new.sh runs both from one resolve_upload_path case. MEASURED: analytics.js 1964 -> 782 B gzipped against its <=1024 B budget — INSIDE BUDGET with 242 B of headroom, better than the ~972 B this entry predicted because the ledger #44 focus-block move shed bytes too. photo-resize.js 2042 -> 1492 B against 2048 B, turning six bytes of headroom into 556. Also site.js 1672 -> 550 B and form-validate.js 2379 -> 783 B. The JS stripper carries one rule the CSS one does not need: it REFUSES (exit 2) rather than guess at a regex literal, because guessing wrong truncates a file silently; a refusal lands in deploy-new.sh's existing fail-open branch and ships the source unchanged. scripts/strip-js-selftest.sh proves it: 21/21, including ASI newline preservation, comment openers inside strings and template literals, division-vs-regex on photo-resize.js's real expressions, refusal on three regex positions, and every file in src/js/ still parsing after stripping. | fixed |  | 2026-09-21T18:08:17.794Z |  | 2026-09-24T09:10:00.000Z |
| 44 | 04 | deviation | src/js/analytics.js |  | CLOSED 2026-09-24 — the two-line move is done. The UI-SPEC C-8 error-band .focus() left src/js/analytics.js for src/js/site.js. Rationale as recorded: content blockers commonly match the filename 'analytics.js' by pattern, so a blocked file silently stopped error-band focus for exactly the keyboard and screen-reader users who need it, landing them at the top of the document instead of on the explanation of why their enquiry failed. TWO IMPLEMENTATION NOTES worth keeping. (1) It is its OWN IIFE in site.js, not folded into the existing one — that one returns early on `if (!nav)`, and error-band focus must not acquire a hidden dependency on a nav element being present. Its only guard is the band's own existence. (2) A pointer comment is left at the old site in analytics.js so the next reader does not re-add it there. Both files pass node --check. | fixed |  | 2026-09-21T18:08:25.653Z |  | 2026-09-24T09:10:00.000Z |
...
| 56 | 04 | unrun-verify | scripts/cutover-sweep.sh |  | PARTIALLY CLOSED 2026-09-24 — the STATIC-ASSET half is now a gate; the PHP-RENDERED half is NOT, and is raised as entry 57. What shipped: cutover-sweep.sh section [3c] compares the bytes the origin serves against the bytes the tree would upload, for all 103 static assets, reusing the [3b] fetch so it costs no extra request. The comparison is against the WIRE FORM, not the source — deploy-new.sh strips comments from CSS and (since #43) JS, so a naive digest would fail permanently on every stylesheet and every script, and a gate that always fails gets switched off. The expected bytes come from the new scripts/lib/wire-form.sh, THE SAME function deploy-new.sh now calls to decide what it uploads, so the two cannot drift — that shared definition, not the digest, is the actual fix. PROVEN IN BOTH DIRECTIONS AGAINST STAGING, per the lesson recorded in entry 55. True positive: the run reported 4 of 103 STALE — js/analytics.js 1799 B local vs 4042 B served, js/site.js 1406 vs 2476, js/form-validate.js 1855 vs 4957, js/photo-resize.js 3725 vs 4645 — all genuinely stale, since JS stripping and the #44 focus move had not been redeployed. Negative control: the other 99 matched byte-for-byte, INCLUDING ALL FOUR STRIPPED STYLESHEETS, which is exactly where a naive digest would have produced permanent false failures. A BUG WAS FOUND BY WRITING THE TEST, and it was the dangerous kind: wire-form.sh resolved its own directory from BASH_SOURCE at CALL time, which is EMPTY under zsh, so the stripper path resolved against the caller's cwd, was not found, and the function fell into its 'ships unchanged' branch — A MISSING TRANSFORM WAS INDISTINGUISHABLE FROM A LEGITIMATE ANSWER, and the sweep would have confidently called all four stylesheets stale on that basis. Now resolved once at source time, with a distinct rc 2 meaning 'the transform is broken, nothing is proven', which [3c] reports as its own failure rather than folding into the stale count. Verified by hiding the stripper and re-running: 4 CSS report 'could not be compared AT ALL', not 4 stale. Locked in by strip-js-selftest.sh (24/24) which asserts wire-form under BOTH bash and zsh and asserts the rc-2 path, restoring the moved file from a trap so an interrupt cannot leave the repo stripper-less. | fixed |  | 2026-09-24T07:05:00.000Z |  | 2026-09-24T10:00:00.000Z |
| 57 | 04 | unrun-verify | scripts/cutover-sweep.sh |  | A STALE PHP-RENDERED PAGE STILL PASSES EVERY CHECK. This is the half of entry 56 that its [3c] deliberately does not cover, split out so it is not assumed closed. The .html pages EXECUTE on this host, so served bytes never equal source bytes and no digest can compare them. A stale includes/footer.php or includes/header.php on the origin renders stale markup on all 20 pages and passes [3b] (the file exists), [3c] (not compared — it is not a static asset) and [9] (the page renders Cyrillic prose, an h1, no diagnostics, no widget). THIS IS THE LARGER HALF OF WHAT ACTUALLY HAPPENED ON 2026-09-23: the footer band the owner had removed was still live on all 20 pages because the include had not been redeployed. [3c] would NOT have caught it. Neither would anything else in the tree today. WHY IT IS NOW TRACTABLE AND WAS NOT BEFORE: a PHP 8.5.10 runtime exists on the build machine as of 2026-09-24, so each page can be rendered LOCALLY through php -S and compared against the served body — which was impossible while the phase had no php binary. SUGGESTED SHAPE: render locally, then normalise the small set of legitimately-dynamic tokens before comparing — the ?v=<filemtime> asset stamps (local mtime != server mtime by construction), the signed render timestamp on kontakti.html, and the base URL (staging path segment vs root). Everything else should be byte-identical, and any residue is either a real staleness or a dynamic token worth naming explicitly. VALIDATE IT THE WAY 56 WAS VALIDATED: it must go green on a freshly-deployed staging AND red against a deliberately-stale include, or it proves nothing. Do NOT ship it as a gate on the strength of a synthetic test alone — entry 55 records a check that passed synthetically and then failed 19 of 20 real pages on a false positive. | open |  | 2026-09-24T10:00:00.000Z |  |
| 58 | 04 | deferred | src/uslovia.html |  | uslovia.html IS MISSING EVERY GDPR ARTICLE 13 RIGHTS DISCLOSURE. Owner reviewed the wording at GATE 0.4 on 2026-09-24, approved what is there, and DELIBERATELY DEFERRED this to post-launch — it is a logged decision, not an oversight. Recorded so it is not lost. WHAT IS PRESENT AND GOOD: the page names Торин Къмпани ООД as the collecting entity, states what is collected (name, phone, email, device model, fault description, optional photos), why, and that no enquiry register is kept server-side. WHAT IS ABSENT, all of it mandatory under Art. 13(2): right of access, rectification, erasure, restriction, objection, portability; RIGHT TO WITHDRAW CONSENT; and the right to lodge a complaint with КЗЛД. The withdrawal one is the sharpest because the contact form carries a consent checkbox, so consent IS the legal basis being relied on, with no stated way to withdraw it. Also absent: ЕИК and седалище anywhere in the tree (ЗЕТ чл.4 trader-identification duty) — the address 'ул. Свети Иван Рилски 46, София 1606' and office@torin.bg are in site-config.php but no registration number exists. NOT CREATED BY THE CUTOVER: the live site has these same gaps today, so the swap neither introduces nor worsens the exposure — which is the basis on which deferring it was reasonable. TO CLOSE: add a «Вашите права» section in the page's existing voice, plus ЕИК in the footer or contact page, and have someone qualified read it. General terms (общи условия) are NOT owed and were checked: they bind distance contracts, and this site sells nothing, takes no payment and holds no accounts; the shop's real terms are the warranty ones on warrently.html. | open | owner deferred at GATE 0.4, 2026-09-24 | 2026-09-24T12:00:00.000Z |  |
| 59 | 04 | deferred | src/uslovia.html |  | uslovia.html says «сайта www.torin.bg» while the canonical host is becoming the bare apex. One occurrence. Cosmetic, but this page becomes a public commitment the moment it is served from the root, and D4-29 makes https://torin.bg (no www) canonical — so the document names a hostname the site is about to redirect away from. Fix with the GDPR rights work in entry 58 rather than as a separate pass. | open | owner deferred at GATE 0.4, 2026-09-24 | 2026-09-24T12:00:00.000Z |  |
...
[
  ...
  },
  {
    ...
    "file": "src/includes/spam-guard.php",
    "line": null,
    "description": "CLOSED 2026-09-24 as STALE — it WAS authored. Commit c6cce02 'test(spam-guard): the behaviour block, executable' wrote scripts/spam-guard-selftest.php with 27 assertions, not the 9 the cutover checklist predicted. Executed for the first time on 2026-09-24 against the new PHP 8.5.10 runtime: 27/27 hold, covering the decoy field (filled/untouched/absent/array-posted), timestamp signing (forged, same-length tampered, malformed, too-fast, stale, inclusive bounds), the per-address throttle (unseen address, hashed record, inside-window refusal, outside-window pass, per-address isolation, max=0 proving the counter is read), and source-level assertions (hash_equals, no session, no superglobal in logs, no forwarded-for trust, PHP 5.2 short-array contract, no residue left behind). With this, ALL FOUR selftests named in cutover checklist 1.2 exist and pass: upload 18/18, settings 27/27, notify 9/9, spam-guard 27/27 — 81 assertions, zero unrun.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-09-21T06:08:41.039Z",
    "resolved_at": "2026-09-24T09:10:00.000Z"
  },
  {
    "id": 36,
    "kind": "deviation",
  ...
  },
  {
    ...
    "line": null,
    "description": "CLOSED 2026-09-24 — decided (option b) and implemented. Owner picked deploy-time JS comment stripping over raising the budget or hand-stripping. scripts/lib/strip-js-comments.py now mirrors the existing strip-css-comments.py and deploy-new.sh runs both from one resolve_upload_path case. MEASURED: analytics.js 1964 -> 782 B gzipped against its <=1024 B budget — INSIDE BUDGET with 242 B of headroom, better than the ~972 B this entry predicted because the ledger #44 focus-block move shed bytes too. photo-resize.js 2042 -> 1492 B against 2048 B, turning six bytes of headroom into 556. Also site.js 1672 -> 550 B and form-validate.js 2379 -> 783 B. The JS stripper carries one rule the CSS one does not need: it REFUSES (exit 2) rather than guess at a regex literal, because guessing wrong truncates a file silently; a refusal lands in deploy-new.sh's existing fail-open branch and ships the source unchanged. scripts/strip-js-selftest.sh proves it: 21/21, including ASI newline preservation, comment openers inside strings and template literals, division-vs-regex on photo-resize.js's real expressions, refusal on three regex positions, and every file in src/js/ still parsing after stripping.",
    "status": "fixed",
    "reason": "",
  ...
  },
  {
    ...
    "file": "src/js/analytics.js",
    "line": null,
    "description": "CLOSED 2026-09-24 — the two-line move is done. The UI-SPEC C-8 error-band .focus() left src/js/analytics.js for src/js/site.js. Rationale as recorded: content blockers commonly match the filename 'analytics.js' by pattern, so a blocked file silently stopped error-band focus for exactly the keyboard and screen-reader users who need it, landing them at the top of the document instead of on the explanation of why their enquiry failed. TWO IMPLEMENTATION NOTES worth keeping. (1) It is its OWN IIFE in site.js, not folded into the existing one — that one returns early on `if (!nav)`, and error-band focus must not acquire a hidden dependency on a nav element being present. Its only guard is the band's own existence. (2) A pointer comment is left at the old site in analytics.js so the next reader does not re-add it there. Both files pass node --check.",
    "status": "fixed",
    "reason": "",
  ...
  },
  {
    ...
    "line": null,
    "description": "PARTIALLY CLOSED 2026-09-24 — the STATIC-ASSET half is now a gate; the PHP-RENDERED half is NOT, and is raised as entry 57. What shipped: cutover-sweep.sh section [3c] compares the bytes the origin serves against the bytes the tree would upload, for all 103 static assets, reusing the [3b] fetch so it costs no extra request. The comparison is against the WIRE FORM, not the source — deploy-new.sh strips comments from CSS and (since #43) JS, so a naive digest would fail permanently on every stylesheet and every script, and a gate that always fails gets switched off. The expected bytes come from the new scripts/lib/wire-form.sh, THE SAME function deploy-new.sh now calls to decide what it uploads, so the two cannot drift — that shared definition, not the digest, is the actual fix. PROVEN IN BOTH DIRECTIONS AGAINST STAGING, per the lesson recorded in entry 55. True positive: the run reported 4 of 103 STALE — js/analytics.js 1799 B local vs 4042 B served, js/site.js 1406 vs 2476, js/form-validate.js 1855 vs 4957, js/photo-resize.js 3725 vs 4645 — all genuinely stale, since JS stripping and the #44 focus move had not been redeployed. Negative control: the other 99 matched byte-for-byte, INCLUDING ALL FOUR STRIPPED STYLESHEETS, which is exactly where a naive digest would have produced permanent false failures. A BUG WAS FOUND BY WRITING THE TEST, and it was the dangerous kind: wire-form.sh resolved its own directory from BASH_SOURCE at CALL time, which is EMPTY under zsh, so the stripper path resolved against the caller's cwd, was not found, and the function fell into its 'ships unchanged' branch — A MISSING TRANSFORM WAS INDISTINGUISHABLE FROM A LEGITIMATE ANSWER, and the sweep would have confidently called all four stylesheets stale on that basis. Now resolved once at source time, with a distinct rc 2 meaning 'the transform is broken, nothing is proven', which [3c] reports as its own failure rather than folding into the stale count. Verified by hiding the stripper and re-running: 4 CSS report 'could not be compared AT ALL', not 4 stale. Locked in by strip-js-selftest.sh (24/24) which asserts wire-form under BOTH bash and zsh and asserts the rc-2 path, restoring the moved file from a trap so an interrupt cannot leave the repo stripper-less.",
    "status": "fixed",
    "reason": "",
  ...
  },
  {
    ...
    "file": "scripts/cutover-sweep.sh",
    "line": null,
    "description": "A STALE PHP-RENDERED PAGE STILL PASSES EVERY CHECK. This is the half of entry 56 that its [3c] deliberately does not cover, split out so it is not assumed closed. The .html pages EXECUTE on this host, so served bytes never equal source bytes and no digest can compare them. A stale includes/footer.php or includes/header.php on the origin renders stale markup on all 20 pages and passes [3b] (the file exists), [3c] (not compared — it is not a static asset) and [9] (the page renders Cyrillic prose, an h1, no diagnostics, no widget). THIS IS THE LARGER HALF OF WHAT ACTUALLY HAPPENED ON 2026-09-23: the footer band the owner had removed was still live on all 20 pages because the include had not been redeployed. [3c] would NOT have caught it. Neither would anything else in the tree today. WHY IT IS NOW TRACTABLE AND WAS NOT BEFORE: a PHP 8.5.10 runtime exists on the build machine as of 2026-09-24, so each page can be rendered LOCALLY through php -S and compared against the served body — which was impossible while the phase had no php binary. SUGGESTED SHAPE: render locally, then normalise the small set of legitimately-dynamic tokens before comparing — the ?v=<filemtime> asset stamps (local mtime != server mtime by construction), the signed render timestamp on kontakti.html, and the base URL (staging path segment vs root). Everything else should be byte-identical, and any residue is either a real staleness or a dynamic token worth naming explicitly. VALIDATE IT THE WAY 56 WAS VALIDATED: it must go green on a freshly-deployed staging AND red against a deliberately-stale include, or it proves nothing. Do NOT ship it as a gate on the strength of a synthetic test alone — entry 55 records a check that passed synthetically and then failed 19 of 20 real pages on a false positive.",
    "status": "open",
    "reason": "",
  ...
  },
  {
    ...
    "file": "src/uslovia.html",
    "line": null,
    "description": "uslovia.html IS MISSING EVERY GDPR ARTICLE 13 RIGHTS DISCLOSURE. Owner reviewed the wording at GATE 0.4 on 2026-09-24, approved what is there, and DELIBERATELY DEFERRED this to post-launch — it is a logged decision, not an oversight. Recorded so it is not lost. WHAT IS PRESENT AND GOOD: the page names Торин Къмпани ООД as the collecting entity, states what is collected (name, phone, email, device model, fault description, optional photos), why, and that no enquiry register is kept server-side. WHAT IS ABSENT, all of it mandatory under Art. 13(2): right of access, rectification, erasure, restriction, objection, portability; RIGHT TO WITHDRAW CONSENT; and the right to lodge a complaint with КЗЛД. The withdrawal one is the sharpest because the contact form carries a consent checkbox, so consent IS the legal basis being relied on, with no stated way to withdraw it. Also absent: ЕИК and седалище anywhere in the tree (ЗЕТ чл.4 trader-identification duty) — the address 'ул. Свети Иван Рилски 46, София 1606' and office@torin.bg are in site-config.php but no registration number exists. NOT CREATED BY THE CUTOVER: the live site has these same gaps today, so the swap neither introduces nor worsens the exposure — which is the basis on which deferring it was reasonable. TO CLOSE: add a «Вашите права» section in the page's existing voice, plus ЕИК in the footer or contact page, and have someone qualified read it. General terms (общи условия) are NOT owed and were checked: they bind distance contracts, and this site sells nothing, takes no payment and holds no accounts; the shop's real terms are the warranty ones on warrently.html.",
    "status": "open",
    "reason": "owner deferred at GATE 0.4, 2026-09-24",
  ...
  },
  {
    ...
    "file": "src/uslovia.html",
    "line": null,
    "description": "uslovia.html says «сайта www.torin.bg» while the canonical host is becoming the bare apex. One occurrence. Cosmetic, but this page becomes a public commitment the moment it is served from the root, and D4-29 makes https://torin.bg (no www) canonical — so the document names a hostname the site is about to redirect away from. Fix with the GDPR rights work in entry 58 rather than as a separate pass.",
    "status": "open",
    "reason": "owner deferred at GATE 0.4, 2026-09-24",
    "recorded_at": "2026-09-24T12:00:00.000Z",
    "resolved_at": null
