...
> 3. **`kat-6` is unpublished and `problem-stari.html` is orphaned.** The page is live and correct but has no typed inbound link, because `torin_category_href()` routes cards to `index.html#kat-6` while `'published' => false`. Stripping `X-Robots-Tag: noindex` at cutover makes it indexable *and* still unreachable from navigation — the same shape as the defect that hid on `remont-na-portove.html` for an entire phase. **Best resolved BEFORE cutover by simply publishing it:** the page renders 845 Cyrillic tokens (longer than `about`, `ekran-klaviatura-portove` and `index`, all published), every claim traces to an owner answer, the flag is reversible, and on staging it is free to flip. See OWNER-QUESTIONS #3 — read the **AMENDED** block, which supersedes the first recommendation.
