...
> **⚠ Carried into Phase 4 from 3.5 — decide these at cutover, do not let them default:**
> 1. **`.htaccess` needs TWO edits, not one** — the canonicalisation target *and* the `RewriteBase`. Shipping one without the other 301s four indexed URLs to a 404 while still returning a correct-looking `301` + `Location`. That is not hypothetical; it is what happened on the first deploy of the retirement rules.
> 2. **No host canonicalisation exists at the domain root.** All four protocol/host variants return 200 with no redirect, and GSC shows the ranking variant is `http://www.torin.bg/` — the least preferred form — while `site-config.php` sets `base_url` to `https://torin.bg/`. SEO-04 covers paths; nothing covers the host, which is how this went unseen for three phases.
> 3. ~~**`kat-6` is unpublished and `problem-stari.html` is orphaned.**~~ **✅ RESOLVED 2026-09-16 — published and deployed.** `kat-6` is `'published' => true`; `torin_category_href()` now routes cards and nav to `problem-stari.html`, and zero `index.html#kat-6` stubs remain. Verified live after the shared-include deploy: all 19 pages `200 warn=0`, homepage probe PASS (`sectionCount: 6`, `ratingBadgePresent: true`, `brandItemCount: 7`, `adjacentTintedPairs: 0`), `svc-page` probe on the newly-routed page PASS. The orphaned-page trap is retired before cutover rather than at it. **Still open on that page:** the `[ASSUMED]` symptoms line is now publicly visible (#3f, #16), and #3c / #32 / #33 remain unanswered — see OWNER-QUESTIONS #3, AMENDED block.
> 4. **Staging still holds files the deploy script cannot remove** — the four retired `.html` pages and three withdrawn photographs. `deploy-new.sh` uploads and never deletes; clearing them is a manual FileZilla pass.
**Depends on**: Phase 3
...
- [x] 04-02-PLAN.md — **TRACER:** one enquiry wired end to end — contact page → handler → notification (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 04-03-PLAN.md — Upload pipeline, photo delivery and the browser-side downscale (wave 3)
...
- [x] 04-07-PLAN.md — Analytics instrumentation and design-system cleanup (wave 6)

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 04-08-PLAN.md — WebP siblings, cache lifetimes, `robots.txt` and `sitemap.xml` (wave 7)

**Wave 8** *(blocked on Wave 7 completion)*

- [x] 04-09-PLAN.md — Cutover prep: canonical-host decision, `.htaccess` promotion diff, go/no-go sweep (wave 8)
