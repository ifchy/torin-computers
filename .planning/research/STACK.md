...
**Domain:** Small local-business service website (computer/laptop/electronics repair shop), content-heavy, no e-commerce, redesign of an existing static site
**Researched:** 2026-08-04
**Confidence:** MEDIUM-HIGH (current library versions verified via web search on 2026-08-04; architectural recommendations are well-established patterns, not bleeding-edge/speculative)

## Recommendation: Reskin vs Full Rebuild

**Verdict: Full rebuild with a static site generator (Astro), not a reskin of the Liquid template.**

The current site's core maintainability problem isn't its visual style — it's that ~17 HTML pages each hand-duplicate the full `<head>`, nav, header, and footer markup, plus a jQuery/Bootstrap-era vendor stack (jQuery UI, ScrollMagic, pagePiling — a scroll-hijacking full-page-slide library that actively hurts mobile usability and Core Web Vitals). Reskinning in place means every future edit to the nav or footer still has to be repeated across 17 files by hand, and "modern" is capped by how far you can push a template built for 2018-era design trends. A rebuild with an SSG fixes the duplication permanently (shared layout component, service pages as data), gets fast/modern by default, and — critically — is **still 100% deployable to plain FTP shared hosting**, because the build step runs on the developer's machine or in CI, never on the host. The host only ever receives static HTML/CSS/JS files, identical in kind to what it serves today.
...
Given the explicit goals (look modern, be fast, competitive gap-closing, easier maintenance) and that this is a from-scratch redesign rather than a quick patch, the rebuild is the right call. The reskin path is documented below as a legitimate fallback if timeline becomes the binding constraint — see "Stack Patterns by Variant."

**A third option — hybrid — is what "reskin" should mean if chosen:** don't just restyle the existing HTML files; first refactor them into PHP includes (`header.php`/`footer.php`/`nav.php`) since the host already runs PHP (confirmed by the existing `mailer.php`). That alone kills the 17-page duplication problem with zero new tooling. This is the "cheap 80%" version of what the SSG gives you, and is called out explicitly below because it's a materially better reskin than "edit each HTML file."
...
npm create astro@latest torin-site
...
**If reskin in place (fallback, timeline-constrained):**
...
---
*Stack research for: small-business local-service website redesign (Bulgarian computer/laptop repair shop), plain-FTP shared-hosting deployment*
*Researched: 2026-08-04*
