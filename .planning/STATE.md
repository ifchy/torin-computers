...
- [Phase 4 — ✅ THE DELIVERY CASCADE HAS RUN FOR REAL, 2026-09-22]: **The owner submitted the contact form and all three channels delivered** — Telegram message, shop email, and the customer confirmation email. This is the one class of evidence nothing in this project can produce on its own: every prior check either ran a selftest with no network or asserted on an API's *answer* rather than on what a human received, and `--submit` pages a person so it is owner-triggered only. **Log evidence**, `public_html/new/error_log` via `scripts/fetch-remote.sh`, correlation id **`f4e59592`**, 19:33:47–19:33:50 Europe/Sofia: both mail legs logged `mail no smtp credential, using sendmail` then `mail delivered via=sendmail`, **exactly once each** (owner notification + customer confirmation), `photos=0 leftover=0`, no telegram failure line, whole cascade **~3s**. This proves **ledger 38 case (a)** — the shipped no-credential state — including its hard exactly-once condition, and **closes ledger 42** (deny block now 403 on all three vendored PHPMailer files; its open latency question answered by the same run). Ledger 38 stays **open** for cases (b) and (c), which are the ones that exercise an actual *failure* falling through.
- [Phase 4 — WHAT THE LIVE PROOF DOES NOT CLOSE]: A success-path run says nothing about a failure path, and a green end-to-end test is precisely what makes people assume the rest is fine. **Still open, unchanged:** ledger **15/40** — the all-channels-failed page has never been exercised; this drove the success branch only. Ledger **17** — the portrait-orientation EXIF path ran on neither side, because `photos=0`; it still needs one portrait photograph from a real handset. Ledger **18** — nobody has watched a photo *render* in Telegram; the owner saw a message, but the open questions are whether a single photo carries the enquiry as its caption and whether three arrive as one media group. Ledger **14** — the honeypot autofill false-positive is unknown, because it was not recorded whether autofill was active; a false positive there silently discards a real enquiry and is invisible to both parties, so "it worked once" is not evidence. **And quick task 260919-m0i is a DIFFERENT CODE PATH** — the old site's root `mailer.php`, not `contact-send.php` under `/new/` — so nothing above bears on it either way. *(It was verified separately on 2026-09-23 by raw-header inspection; see its own entry below. The two were proven by different evidence on different days, and neither run says anything about the other.)*
- [Quick 260919-m0i — ✅ VERIFIED AND CLOSED 2026-09-23]: **The live `mailer.php` CRLF header-injection patch is proven.** The owner read the raw headers of all three test messages at `office@torin.bg` and all three met their criteria: message 1 (13:14:58Z, pre-patch control) **carried** `X-Torin-Injection-Test: PRE-PATCH-260919`; message 2 (13:15:26Z) had the correct `From:`, a `Reply-To:` holding the visitor address and no `X-Torin-`; message 3 (13:15:36Z, injection attempt) returned **zero** `X-Torin` hits with the correct `From:` and no `Reply-To:`. **Message 1 is the load-bearing result:** its criterion is a PRESENCE while message 3's is an ABSENCE, and had the control come back clean, message 3 would have proven nothing — an injection that never worked is absent from every message, patched or not. **Nothing cheaper could have substituted:** an injection POST returned a byte-identical `302` / `location: msg.html` / `content-length: 0` before and after the patch, so no HTTP assertion and no source gate could distinguish patched from unpatched. Message 2 also confirms Reply still addresses the customer, which was the thing the change was most likely to break. Full record: `.planning/quick/260919-m0i-fix-live-mailer-header-injection/260919-m0i-EVIDENCE.md` §5. **Owner housekeeping outstanding:** delete the three `AUTOMATED SECURITY TEST 260919-m0i` messages from the mailbox before they are mistaken for real enquiries. Scope: this is the OLD root `mailer.php`, a different code path from `contact-send.php` under `/new/`, and 04-05 retires it at cutover.
- [Quick 260919-m0i — process, LESSON STANDS]: **Unparsed PHP reached the live root.** No `php` binary on the build machine and Docker down, so the patch was never syntax-checked before upload — the first thing to execute it was a real public HTTP request. It parsed clean, but that is verification *after* exposure, and a parse error would have taken the live contact form down for every visitor. **The capability gap is now closed: PHP 8.5.10 is installed** (quick `260922-i38`, which linted all 45 files clean and found a real containment bug on a selftest's first-ever run). **The rule is NOT retired by the verification above** — lint before a live PHP deploy, every time. 04-09/04-10 deploy PHP to production repeatedly.

### Quick Tasks Completed

| # | Description | Date | Commit | Status | Directory |
|---|-------------|------|--------|--------|-----------|
| 260919-m0i | Patch live `mailer.php` CRLF mail-header injection | 2026-09-19 | `ff7d469` | ✅ **Complete — VERIFIED 2026-09-23** by owner inspection of raw email headers. All three test messages pass: control (msg 1) **carried** the injected `X-Torin-Injection-Test` header, post-patch injection (msg 3) returned **zero** hits, and msg 2's `Reply-To:` still addresses the customer. The control's PRESENCE is what makes msg 3's ABSENCE meaningful — an injection that never worked is absent from every message. No HTTP assertion could have settled it: the injection POST returned a byte-identical `302` before and after the patch. ⚠ Owner housekeeping: delete the three `AUTOMATED SECURITY TEST` messages from the mailbox | `.planning/quick/260919-m0i-fix-live-mailer-header-injection/` |
...
| 260922-dx2 | Sequence the sitemap regeneration into the cutover checklist (#51) | 2026-09-22 | `bfb8229`, `9379900` | ✅ **Complete** — checklist step 2.0 added (open 36→35). Found and fixed en route: regeneration emitted 20 locs including the GSC verification token; both sitemap scripts now exclude non-pages, output byte-identical to the committed file. ⚠ **Reported, unfixed:** `.htaccess` is NOT actually excluded from `deploy-new.sh` despite ledger 52 claiming it is | `.planning/quick/260922-dx2-cutover-sitemap-regeneration-step/` |
| 260922-hbk | Enforce `.htaccess` exclusion in `deploy-new.sh` (ledger 52 was unenforced) | 2026-09-22 | `d913c07` | ✅ **Complete** — root `.htaccess` now skipped on a no-arg run and a hard error when named; override `TORIN_DEPLOY_HTACCESS=1`. First cut matched basename and wrongly blocked `vendor/phpmailer/.htaccess` (the ledger-42 deny block) — caught by testing, predicate narrowed to the root path | `.planning/quick/260922-hbk-deploy-htaccess-guard/` |
| 260922-i38 | Install PHP 8.5.10; run the three selftests that had never executed | 2026-09-22 | `a9d9997` | ✅ **Complete** — `php -l` clean on all 45 files; settings 26/26, notify 9/9, upload 4/10→10/10. Closed #19/#29/#30, raised #54 (open 35→33). Found and fixed a real containment-guard bug on its first-ever run | `.planning/quick/260922-i38-php-runtime-selftests/` |
| 260922-onz | Remove the footer notice band (owner decision) | 2026-09-22 | `2eb5f08` | ✅ **Complete** — duplicate of the hours line 34 lines below it; descended from the *temporary* «НОВО Работно време» half of otpuska.js. Closed ledger 33 (open 31→30). 21 pages render clean | `.planning/quick/260922-onz-remove-footer-notice-band/` |
| 260923-ebg | Record the live end-to-end contact-form delivery proof | 2026-09-23 | `c784347` | ✅ **Complete** — owner submitted the real form; Telegram, shop email and customer confirmation all delivered. Log `f4e59592` shows both mail legs `via=sendmail`, **exactly once each**, ~3s, `photos=0`. Proves **ledger 38 case (a)** incl. its exactly-once condition (entry stays **open** for cases b/c — the actual failure fall-through). **Closes ledger 42** (403 on all three vendored PHPMailer files; latency note answered). open 30→29, fixed 21→22. Records what it does NOT close: 15/40, 17, 18, 14, and 260919-m0i (different code path) | `.planning/quick/260923-ebg-record-the-live-end-to-end-contact-form-/` |
| 260922-rmw | Confirmation page — owner's copy, and legible prose | 2026-09-22 | `0e9aa02`, `94bfb51` | ✅ **Complete and live-verified** — owner copy applied verbatim. Two separate causes behind "too light, no spacing": `.svc__intro` is `--c-ink-muted` (6.08:1) carrying the page's whole message, and the second `<p>` had no class so `base.css p { margin: 0 }` left a 0px gap (`.svc__block p` applies only inside `.svc__block`). Fixed via a `.confirm` page scope — **`.svc__intro` deliberately NOT recoloured**, it is the muted lede on 15+ service pages. Live: both paragraphs `rgb(31,42,60)`, 16px gap, `.svc__intro` still muted on 4 sampled service pages. ⚠ The new copy **asserts** the confirmation email where the old hedged it — see SUMMARY for the bounded principle | `.planning/quick/260922-rmw-improve-msg-html-confirmation-copy-and-p/` |
| 260922-q0v | Tidy the contact form — placeholders, submit-time validation | 2026-09-22 | `9a36653`, `1759838`, `d684709` | ✅ **Complete and live-verified 2026-09-22** — owner reported "too much information". Secondary text 14 lines → 2. **Root cause was a bug, not a preference: all 7 `.field__error` nodes carried `hidden` and all 7 rendered** — `.field__error { display: flex }` outranks the UA `[hidden] { display: none }`, so contact-form.php's documented toggle-the-attribute mechanism had never worked. Fixed globally in base.css. New `js/form-validate.js` validates on submit and clears flags live; emitted **before** photo-resize.js and using `stopImmediatePropagation()`, without which a rejected submit leaves the button locked disabled | `.planning/quick/260922-q0v-tidy-the-kontakti-contact-form-fix-hidde/` |
| 260922-ozo | Extend the cutover sweep's rendered probe to all 20 pages | 2026-09-22 | `e594567`, `59c346f`, `0e2f787` | ✅ **Complete** — 19 of 20 pages were covered by `expect_status 200` alone, the same check the `$torin_page` «s» collision passed. Now 20/20 rendered in one session, 107–1077 Cyrillic tokens each. Found and fixed en route: **`cdp.onConsoleError` never existed**, so every `"consoleErrors": []` ever recorded was an empty array nothing could fill and the probe's console-failure branch was unreachable — root cause was `connect()` dropping all CDP events (they carry `method`, never `id`) | `.planning/quick/260922-ozo-extend-the-cutover-sweep-rendered-probe-/` |
...
Last session: 2026-09-17T15:10:56.400Z
...
Resume file: .planning/phases/04-hardening-cutover/04-UI-SPEC.md
