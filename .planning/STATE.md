...
- [Phase 4 — WHAT THE LIVE PROOF DOES NOT CLOSE]: A success-path run says nothing about a failure path, and a green end-to-end test is precisely what makes people assume the rest is fine. **Still open, unchanged:** ledger **15/40** — the all-channels-failed page has never been exercised; this drove the success branch only. Ledger **17** — the portrait-orientation EXIF path ran on neither side, because `photos=0`; it still needs one portrait photograph from a real handset. Ledger **18** — nobody has watched a photo *render* in Telegram; the owner saw a message, but the open questions are whether a single photo carries the enquiry as its caption and whether three arrive as one media group. Ledger **14** — the honeypot autofill false-positive is unknown, because it was not recorded whether autofill was active; a false positive there silently discards a real enquiry and is invisible to both parties, so "it worked once" is not evidence. **And quick task 260919-m0i is a DIFFERENT CODE PATH** — the old site's root `mailer.php`, not `contact-send.php` under `/new/` — so it remains live and runtime-unverified.
- [Quick 260919-m0i — LIVE, UNVERIFIED AT RUNTIME]: **The live `mailer.php` CRLF header-injection patch is deployed but nobody has observed it working.** Shipped to `public_html/mailer.php` 2026-09-19; the injection sink is gone from the deployed source and all source gates pass with five firing negative controls. **But an injection POST returns a byte-identical response before and after the patch** (`302` / `location: msg.html` / `content-length: 0` / zero-byte body), so nothing measurable over HTTP closes the gap. Proof requires reading raw headers of three test emails at `office@torin.bg` (UTC 13:14:58 pre-patch control, 13:15:26 post-patch normal, 13:15:36 post-patch injection) — criteria in `.planning/quick/260919-m0i-fix-live-mailer-header-injection/260919-m0i-EVIDENCE.md` §5. **Message 1's marker must be PRESENT**; its absence voids the whole chain and is not good news. Blocked on mailbox access until Monday 2026-09-21. Rollback (hash-checked): `cp backups/20260919T131139Z/public_html/mailer.php site-current/mailer.php && TORIN_LIVE_DEPLOY_CONFIRM=1 scripts/deploy-live.sh mailer.php`.
- [Quick 260919-m0i — process]: **Unparsed PHP reached the live root.** No `php` binary on the build machine and Docker down, so the patch was never syntax-checked before upload; the first thing to execute it was a real public HTTP request. It parsed clean, but that is verification *after* exposure. Get a PHP linter available before 04-09/04-10, which deploy PHP to production repeatedly.
