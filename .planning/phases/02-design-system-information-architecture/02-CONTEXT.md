...
- **D-06:** Use **Inter**, self-hosted, as the single family for both headings and body. Chosen specifically for complete Cyrillic coverage. — **Reversibility:** costly — a font swap after content is written re-flows every page and re-tunes the whole type scale.
- **D-07:** Verify at implementation time that **Bulgarian localized letterforms** (в, г, д, ж, з, и, й, к, л, п, т, ц, ш, щ, ю render differently in Bulgarian vs Russian convention) resolve correctly under `lang="bg"` via the OpenType `locl` feature. This phase sets `lang="bg"` (SEO-02), which is the trigger for those forms.
...
- **D-36:** If the owner later confirms the obligation has expired, retire `covid.html` with a **301 redirect**, never a bare 404.
